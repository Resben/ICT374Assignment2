 ...
 * 				"&" - shell does not wait for the preceeding command to terminate
 * 				";" - shell waits for the preceding command to terminate
 * author:		MA
 * date:		28-Sep-22
...
// process standard in/out redirection in a command
...
void buildCommandArgumentArray(char *token[], Command *cp) {
	int n = (cp->last - cp->first + 1)  // number of tokens in the command
	...
	int start = cp->first + 1; // dont want to include path in arguments
	int i;
	int k = 0;
	for (i=start; i<=cp->last; ++i) {
...
int separateCommands(char *token[], Command command[]) 
{
	...
	int first=0; // points to the first char token of a command
	int last;    // points to the last token of a command
